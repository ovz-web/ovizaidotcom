...
export default function HeroBrutalist({ lang }: HeroBrutalistProps) {
  ...
  return (
    <section className="relative z-10 max-w-xl mx-auto pt-0 pb-0.5 sm:pb-1 px-4 flex flex-col items-center justify-center text-center">
      {/* Eyebrow */}
      <p className="text-[9px] sm:text-[10px] uppercase tracking-[0.18em] sm:tracking-[0.25em] text-gold mb-0.5 sm:mb-1 font-mono font-bold">
        {isFr ? 'CRÉATION VIDÉO IA POUR MARQUES ET ARTISTES' : 'AI VIDEO PRODUCTION FOR BRANDS AND ARTISTS'}
      </p>

      {/* Hero Logo - Enlarged & Cleaned */}
      <div className="relative flex items-center justify-center my-0.5 sm:my-1 overflow-visible">
        <Image
          src="/logo.png"
          alt={isFr ? 'OVIZai — Logo Studio Cinéma & Vidéo IA 4K' : 'OVIZai — 4K AI Cinema & Video Studio Logo'}
          width={240}
          height={240}
          className="h-16 sm:h-48 w-auto object-contain mix-blend-screen"
          priority
        />
      </div>

      {/* Sobriety Main Title */}
      <h1 className="text-base sm:text-2xl lg:text-3xl font-semibold tracking-tight text-fg text-center mb-0.5 sm:mb-1.5 leading-tight sm:leading-snug">
        {isFr ? (
          <>
      ...
      <p className="text-[11px] sm:text-sm text-muted max-w-xs sm:max-w-xl text-center mx-auto mb-1.5 sm:mb-2 leading-tight sm:leading-relaxed font-mono sm:whitespace-nowrap">
